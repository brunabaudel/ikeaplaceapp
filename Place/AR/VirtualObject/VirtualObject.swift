...
extension VirtualObject {
    ...
    /// Loads one model object within `Models.scnassets`.
    static func availableObject(name: String) -> VirtualObject? {
        let modelsURL = Bundle.main.url(forResource: "Models.scnassets", withExtension: nil)!

        let fileEnumerator = FileManager().enumerator(at: modelsURL, includingPropertiesForKeys: [])!

        let virtualObjectURL = fileEnumerator.filter { element in
            let url = element as! URL
            return url.pathExtension == "scn" && url.path.contains(name)
        }.first as! URL

        return VirtualObject(url: virtualObjectURL)
    }
